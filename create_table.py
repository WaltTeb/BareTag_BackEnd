...
con.execute("CREATE TABLE IF NOT EXISTS tags (tag_id TEXT, user_id INTEGER, tag_name TEXT, latitude REAL, longitude REAL, status BOOLEAN, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(user_id) REFERENCES profiles(user_id))")

# creates table to store entire history of every tag
con.execute("CREATE TABLE IF NOT EXISTS tag_locations (location_id INTEGER PRIMARY KEY AUTOINCREMENT, tag_id TEXT, tag_name TEXT, latitude REAL, longitude REAL, mode TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (tag_id) REFERENCES tags(tag_id))")
