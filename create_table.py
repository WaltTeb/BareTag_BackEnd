...
con.execute("CREATE TABLE IF NOT EXISTS tags (tag_id TEXT, user_id INTEGER, tag_name TEXT, latitude REAL, longitude REAL, altitude REAL, status BOOLEAN, created_at TEXT DEFAULT (DATETIME('now', 'localtime')), ble_lat REAL, ble_long REAL, ble_timestamp TEXT, FOREIGN KEY(user_id) REFERENCES profiles(user_id))")
